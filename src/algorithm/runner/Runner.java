...
public class Runner {
	...
	protected Runner(Coordinate origin, ObstacleCollection obs, double radius,
			FinishedNodesList finished) throws PointInObstacleException {

		System.out.println("constructing visibility for: " + origin
				+ ", radius: " + radius);
		...
		this.origin = origin;
		this.radius = radius;
		this.obs = obs.queryObstacles(new Envelope(origin.x - radius, origin.x
				+ radius, origin.y - radius, origin.y + radius));

		if (!obs.pointInObstacle(origin)) {
			this.getNodePoints();

			HelperNodeConstructor helpNoder = new HelperNodeConstructor(origin,
					radius);
			...
			ShadowNodeConstructor shadowNoder = new ShadowNodeConstructor(
					origin, nodes, radius);
	...
	private void constructAlt() {
		...
		for (int i = 0; i < allNodes.size(); i++) {
			...
			if (vis.isVisible(current) && vis.isVisible(next)
					&& !current.equals(next)
					&& next.distance(origin) <= radius + epsilon) {
				result.addTriangle(origin, next, current);
				// System.out.println("next");
			}
			if (vis.isVisible(current) && vis.isVisible(prev)
					&& !current.equals(prev)
	...
	private void constructArcSegments() {
		...
		if (visible.size() > 0) {
			...
			for (int i = 0; i < visible.size(); i++) {
				...
				if (((current.hasShadow() && current.getShadowNode()
						.isBorderNode()) || current.isBorderNode())
						&& ((prev.hasShadow() && prev.getShadowNode()
								.isBorderNode()) || prev.isBorderNode())) {

					if (prev.getPrevNode().equals2D(current)
							|| prev.getNextNode().equals2D(current)) {
				...
				} else if ((current.hasShadow() && !current.getShadowNode()
						.isBorderNode())
						&& (prev.hasShadow() && !prev.getShadowNode()
								.isBorderNode())) {
					// We may need a triangle
					if (current.getPrevNode().equals(prev)
							|| current.getNextNode().equals(prev)) {
						// no triangle needed
					} else if (current.getShadowNode().getPrevNode().equals(
							prev)
							|| current.getShadowNode().getNextNode().equals(
									prev)) {
						// no triangle
					} else if (prev.getShadowNode().getPrevNode().equals(
							current)
							|| prev.getShadowNode().getNextNode().equals(
									current)) {
						// no triangle
					} else if (!isSpecialCase(current, prev)) {
						// neighbors with a visible connection in between
						result.addTriangle(origin, prev.getShadowNode(),
								current.getShadowNode());
					}
	...
	private boolean isSpecialCase(NodePoint current, NodePoint prev) {
		if (current.hasNeighbour(origin) && prev.hasNeighbour(origin)) {
			Point testPoint = FACTORY
					.createPoint(new Coordinate(
							(origin.x + (current.x - origin.x) / 100 + (prev.x - origin.x) / 100),
							(origin.y + (current.y - origin.y) / 100 + (prev.y - origin.y) / 100)));
			if (current.getPolygon() != null
					&& current.getPolygon().contains(testPoint)) {
				...
				if (current.hasShadow()) {
					...
					try {
						if (radius - distance > epsilon) {
							if (!finished
									.isNodeDone(current, radius - distance)) {
								finished.addNode(current, radius - distance);
								Runner recRun = new Runner(current, obs, radius
										- distance, finished);
								this.result.addGeometry(recRun.getResult()
										.getPolygon());
							}
						}
					} catch (Exception e) {
