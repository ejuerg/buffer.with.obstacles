...
public class TestAlgorithmMultiple {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < polys.size(); i++) {
			...
			visPols2.add(new VisualPolygon(polys.get(i).getExteriorRing()
					.getCoordinates(), Color.darkGray, Color.lightGray, 1f));
		}
		...
		while (radius >= 40) {
			...
			Geometry poly = new BufferWithoutObstacles(origin, radius, obs)
					.getResult();
			if (poly != null) {
				for (int i = 0; i < poly.getNumGeometries(); i++) {
					visPols2.add(new VisualPolygon(((Polygon) poly
							.getGeometryN(i)).getExteriorRing()
							.getCoordinates(), new Color(0, color, color - 50),
							new Color(0, color, color - 50), 1f));
				}
			...
			if (poly != null) {
				for (int i = 0; i < poly.getNumGeometries(); i++) {
					...
					for (int j = 0; j < p.getNumInteriorRing(); j++) {
						visPols2.add(new VisualPolygon(p.getInteriorRingN(j)
								.getCoordinates(), new Color(0, color,
				...
				if (result != null) {
					for (int i = 0; i < result.getNumGeometries(); i++) {
						visPols.add(new VisualPolygon(((Polygon) result
								.getGeometryN(i)).getExteriorRing()
								.getCoordinates(), new Color(0, color,
								color - 50), new Color(0, color, color - 50),
								1f));
