...
public class TestPolygonToNodePoints
{
	...
	public static void main(String[] args)
	{
		...
		vPol.add(new VisualPolygon(p1.getExteriorRing().getCoordinates(), Color.darkGray, null, 1f));
		vPol.add(new VisualPolygon(p2.getExteriorRing().getCoordinates(), Color.darkGray, null, 1f));
		vPol.add(new VisualPolygon(buff.getExteriorRing().getCoordinates(), Color.darkGray, null, 1f));
		vPol.add(new VisualPolygon(new PolygonGenerator(new Envelope(new Coordinate(0, 0), new Coordinate(1000, 600)))
				.createPolygon().getExteriorRing().getCoordinates(), Color.orange, null, 1f));
		...
		{
			vNode.add(new VisualPoint(inters.getGeometryN(i).getCoordinate(), Color.red));
		}
		GeometryCollection inters2 = (GeometryCollection) r1.intersection(buff.getBoundary());
		for (int i = 0; i < inters2.getNumGeometries(); i++)
		{
			vNode.add(new VisualPoint(inters2.getGeometryN(i).getCoordinate(), Color.red));
		}
		GeometryCollection inters3 = (GeometryCollection) r2.intersection(buff.getBoundary());
		for (int i = 0; i < inters3.getNumGeometries(); i++)
		{
			vNode.add(new VisualPoint(inters3.getGeometryN(i).getCoordinate(), Color.red));
		}
	...
	public static LinearRing generateLinearRing(int size)
	{
		...
		LinearRing r = new LinearRing(CoordinateArraySequenceFactory.instance().create(coords), new GeometryFactory());
		if (r.isValid())
		{
			return r;
		} else
