...
public class TestShadowNode {
	...
	public static void main(String[] args) {
		...
		/*for (int i = 0; i < helpNodes.size(); i++) {
			visPoints.add(new VisualPoint(helpNodes.get(i).getPrevNode(),
					Color.blue));
		...
		/*for (int i = 0; i < shadowNodes.size(); i++) {
			visPoints.add(new VisualPoint(shadowNodes.get(i).getPrevNode(),
					Color.blue));
		...
		Polygon buff = (Polygon) new GeometryFactory().createPoint(source)
				.buffer(radius, 100);
		visPols.add(new VisualPolygon(buff.getExteriorRing().getCoordinates(),
				Color.pink, null, 1f));
