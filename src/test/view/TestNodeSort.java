...
public class TestNodeSort {
	...
	private static void makeVisPoints(NodePointCollection nodes,
			Vector<VisualPoint> visP) {
		for (int i = 0; i < nodes.size(); i++) {
			NodePoint node = nodes.get(i);
			Coordinate coord = new Coordinate(node.x, node.y);
			visP.add(new VisualPoint(coord, new Color(100,
					255 - (255 / nodes.size() * i), 255 / nodes.size() * i)));
		}
	}
}
